...
function initBaseMessageHandlers() {
      ...
      if (thread) {
        ...
        if (createNewThread) {
          ...
          if (config.responseMessage) {
            ...
            const werbung = "\nSo kannst du diesen Bot in wenigen Minuten kostenlos selbst erstellen: https://youtu.be/SQueiBfbIRg"
            try {
              const postToThreadChannel = config.showResponseMessageInThreadChannel;
              await thread.sendSystemMessageToUser(responseMessage+werbung, { postToThreadChannel });
            } catch (err) {
  ...
  bot.on("messageUpdate", async (msg, oldMessage) => {
    ...
    if (! msg.author.bot && msg.channel instanceof Eris.PrivateChannel) {
      ...
      const editMessage = utils.disableLinkPreviews(`**Der Dude hat seine Nachricht bearbeitet**\n\`B:\` ${oldContent}\n\`A:\` ${newContent}`);
